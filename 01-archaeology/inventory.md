...
| 2 | Nenhum arquivo `.NSM` em [legacy-sifap/natural-programs](legacy-sifap/natural-programs) | **Ausência**: existem programas com cara de online (`CAD*`, `CONS*`), mas nenhuma tela MAP. Ou as telas foram perdidas, ou os `INPUT` são inline. | **Verificado**: todos os programas online usam `INPUT` inline. `CONSBENF.NSP:13` afirma "USE MAP FOR SCREEN LAYOUT", mas o código usa `SET CONTROL 'MB'` + `INPUT` (`CONSBENF.NSP:112-117`). Cabeçalho e código divergem — candidato a mistério. |
